# Build script for Windows status monitor
# Compiles the Go status monitor for Windows

#Requires -Version 5.1

$ErrorActionPreference = "Stop"

Write-Host "Building Mole Status Monitor for Windows..." -ForegroundColor Cyan

# Get project root
$ProjectRoot = Split-Path -Parent (Split-Path -Parent $MyInvocation.MyCommand.Path)
$CmdDir = Join-Path $ProjectRoot "cmd\status"
...
# Build output path
$OutputPath = Join-Path $BinDir "status-go.exe"
...
Write-Host ""
Write-Host "Status monitor ready! Run it with:" -ForegroundColor Cyan
Write-Host "  .\bin\status.ps1" -ForegroundColor White
